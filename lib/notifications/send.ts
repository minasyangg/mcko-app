...
// назначенный группе, приходят с одинаковым заголовком. Источник (программа /
// группа / персонально) нужен и учителю (понять, где искать работу), и ученику
// (понять, что именно искать в своём списке среди похожих заданий).
...
export async function notifyAssignmentCreated(assignmentId: string): Promise<void> {
  try {
    ...
    // числе одинаково озаглавленных заданий (тест из программы и тот же тест
    // назначенный группе отдельно) непонятно, что именно искать в списке.
    // «Персональное назначение» не показываем — банально и так очевидно.
    const sourceLine = ctx.source && ctx.source !== 'Персональное назначение'
      ? `Откуда: ${ctx.source}`
      : null
    const message = lines([
