...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Verify the rule belongs to this teacher's organization
  const { data: rule } = await supabase.from('scoring_rules').select('organization_id').eq('id', id).single()
  if (!rule || rule.organization_id !== profile.organization_id) return Response.json({ error: 'Not found' }, { status: 404 })
