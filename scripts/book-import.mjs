...
function pullContinuation(fromIndex) {
  ...
  for (let j = fromIndex + 1; j < pages.length && budget > 0; j++) {
    ...
    if (first) {
      // решаем по началу первого фрагмента: продолжение задания или теория
      if (THEORY_START.test(cont)) break
      if (!CONT_START.test(cont) && cont.length > 200) break // длинный абзац с заглавной — теория
      first = false
    }
