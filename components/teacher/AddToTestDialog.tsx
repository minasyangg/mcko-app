...
// Универсальный диалог «добавить задание в тест»: работает и для книг,
// и (в будущем) для библиотеки — отличается только addUrl.
      ...
      .then(({ data }) => {
        ...
        for (const t of data ?? []) {
          ...
          if (versions.length > 0) {
            editable.push({ testId: t.id, title: t.title, versionId: versions[0].id })
          }
        }
        setTests(editable)
      })
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Добавить задание {problemLabel} в тест</DialogTitle>
          <DialogDescription>
            Задание будет скопировано в выбранный тест (только черновики и версии на проверке).
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Select value={versionId || '_none'} onValueChange={(v) => setVersionId(v === '_none' ? '' : v)}>
              ...
              <SelectContent>
                <SelectItem value="_none">— Выберите тест —</SelectItem>
                {(tests ?? []).map(t => (
                  <SelectItem key={t.versionId} value={t.versionId}>{t.title}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            {tests !== null && tests.length === 0 && (
              <p className="text-xs text-muted-foreground">
                Нет тестов, доступных для редактирования. Создайте тест в разделе «Тесты».
              </p>
            )}
          </div>
